...
def loadDatadet_4_3(infile):
    ...
    for line in sourceInLine:
        ...
        if sign==1: # bond
            ...
            Dihedral[temp2[0]+temp2[1]+temp2[2]+temp2[3]]=temp2[4:7]
...
def loadDatadet_nonbond(infile):
    ...
    for line in sourceInLine:
        temp1=line.strip('\n')
        first=temp1.split()
        nonbond[first[0]]=[float(first[2]),float(first[3])] # {id:[eps,Rmin/2]}
    return nonbond
...
if __name__ == '__main__':
    ...
    for i in range(len(angle_structure)):
        for j in range(i+1,len(angle_structure)): # c1 c2 c3 - h1 c1 c2
            if angle_structure[i][0]==angle_structure[j][1] and angle_structure[i][1]==angle_structure[j][2]:
                ...
                Vdihedral=kx*(1+math.cos(n*dihedral-e))
                sum=sum+Vdihedral
            ...
            # h1 c1 c2 - c1 c2 h4
                ...
                continue
    print('The sum of dihedral is: '+str(sum))
    ...
    for i in nonbond:
        ...
        U_lj=eps_ab*(math.pow(Rab_2/rab,12)-e*math.pow(Rab_2/rab,6))
        U_coul=332*(1/rab)
